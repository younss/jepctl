...
### Changed
- Companion reworked for a first-run payoff (per design review): a single **Wake the companion** button loads both models, opens camera and microphone, registers the quiet room as neutral and goes interactive; three guided **Teach a trick** cards (clap, wave, body mirror) replace the empty form; a live speech bubble replaces the raw pose numbers; and the senses panel gains an attention reticle on the eye view plus a trigger threshold on the ear scope.
- Robot Twin reworked around use cases (per design review): three cards (Mirror mode / Visual mission / Manual workshop) replace the cryptic mode dropdown (kept under *Advanced modes*), a one-click **Run mirror demo (3 poses)** teaches preset poses with a guided countdown overlay, Visual mission shows **target image vs current vision with a latent-energy dial** (`GET /api/robot/goal-image`), and a plain-language banner explains what the robot is doing in real time.
